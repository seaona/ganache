...
export default class EthereumApi implements Api {
   ...
   */
  @assertArgLength(3)
  async db_putString(dbName: string, key: string, value: string): Promise<boolean> {
    return false;
  }
  ...
  @assertArgLength(0, 1)
  async evm_mine(
    arg?: number | Ethereum.MineOptions
  ): Promise<"0x0"> {
   ...
   */
  @assertArgLength(2)
  async eth_getTransactionByBlockHashAndIndex(hash: DATA, index: QUANTITY): Promise<Ethereum.SignedTransaction<"internal"> | null> {
    const blockchain = this.#blockchain;
   ...
   */
  @assertArgLength(1)
  async eth_getTransactionByHash(transactionHash: DATA): Promise<Ethereum.SignedTransaction<"internal"> | Ethereum.PooledTransaction<"internal"> | null> {
   ...
   */
  @assertArgLength(1)
  async eth_getTransactionReceipt(transactionHash: DATA): Promise<Ethereum.TransactionReceipt<"internal">> {
   ...
   */
  @assertArgLength(1, 2)
  async eth_call(transaction: Ethereum.CallTransaction, blockNumber: QUANTITY | Tag = Tag.latest) {
    const blockchain = this.#blockchain;
  ...
  async txpool_content(): Promise<Ethereum.TransactionPoolContent<"internal">> {
    ...
    const processMap = (map: Map<string, Heap<TypedTransaction>>) => {
      let res: Record<string, Record<string, Ethereum.PooledTransaction<"internal">>> = {};
      for (let [_, { array, length }] of map) {
        for (let i = 0; i < length; ++i) {
          ...
          res[from][nonce] = transaction.toJSON(common) as Ethereum.PooledTransaction<"internal">;
        }
      }
      return res;
    };
